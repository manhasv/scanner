...
class PreprocessConfig:
    """Relative scaling factors for resolution-independent OpenCV kernels."""
    ILLUMINATION_BLUR = 0.15 #0.12  # Large blur to extract background lighting
    ADAPTIVE_BLOCK = 0.008 # Medium block to read local text/shadow differences
    MORPH_CLOSE = 0.06 #0.005       # Tiny kernel to bridge pixel gaps in edges
...
def _normalized(lightness: np.ndarray, base_dim: int) -> np.ndarray:
    """"""
    #Illumination Normalization via float division with Gaussian background
    light_float = lightness.astype(np.float32)
    blur_size = _odd(int(base_dim * PreprocessConfig.ILLUMINATION_BLUR), minimum=31)
...
def prepare_detection_artifacts(
    ...
    normalized = _normalized(lightness, base_dim)
    ...
    close_size = _odd(int(base_dim * PreprocessConfig.ADAPTIVE_BLOCK), minimum=3)
    ...
    block_size = _odd(int(base_dim * PreprocessConfig.MORPH_CLOSE), minimum=31)
