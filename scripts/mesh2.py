...
import scipy.interpolate as interp # Required for B-Spline mesh generation
...
def preprocess(image):
    ...
    # 1. Gaussian Blur: Critical to remove high-frequency noise (like text) 
    # so Canny doesn't get confused and trace the letters.
    blurred = cv2.GaussianBlur(gray, (7, 7), 0)
    
    # 2. Canny Edge Detection: Finds the sharp gradients (the edges of the paper).
    edges = cv2.Canny(blurred, 50, 150)
    
    # 3. Morphological Dilation: Thickens the edges to close any tiny gaps.
    # Without this, findContours might see a broken line instead of a closed shape.
...
def resample_curve(points, num_points):
    ...
    # --- FIX: Downsample to remove high-frequency pixel noise ---
    # We take roughly 30 evenly spaced points along the raw edge. 
    # This prevents the B-spline from overfitting to the jagged pixel steps.
    ...
    # Set s (smoothing) to a slightly higher value (10.0 instead of 0) to allow 
    # the spline to relax into a natural curve rather than violently snapping.
    tck, u = interp.splprep([x, y], s=10.0, k=k)
